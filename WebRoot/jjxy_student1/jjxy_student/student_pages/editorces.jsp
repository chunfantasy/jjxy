...
%>
		 ...
		 <form name="form1" method="post" action="papersubmit.jsp?action=<%=action.equals("submit")?"submit":"editor"%>">
              ...
              <TR>
                <TD bgColor=#75c6c6>
                  <TABLE cellSpacing=1 cellPadding=4 width="100%" border=0>
                    ...
                    <TD bgColor=#ffffff>
					<%if(action.equals("submit")){%><%=rs.getString("FormatStart")%><%=rs.getString("FormatEnd")%>
					<%} else if(action.equals("editor")){%>
					<%=rs.getString("FormatStart")%><%String content=rs.getString("Content");content=content.replaceAll("<br>","\n");%><%=content%><%=rs.getString("FormatEnd")%><%} else{%><%=rs.getString("Content")%><%}%></TD></TR>
					<% }
                     %>
					</TBODY></TABLE></TD></TR></TBODY></TABLE><BR>
            <CENTER><%if(action.equals("submit")||action.equals("editor")){%><input type="hidden" name="action" value="submit"><input type="hidden" name="count" value="<%=i%>"><input type="hidden" name="fileid" value="<%=fileid%>"><input type="hidden" name="filename" value="<%=filename%>"><input type="hidden" name="paperid" value="<%=paperid%>"><INPUT type=submit value="提 &nbsp; 交" name=submit>&nbsp;&nbsp;&nbsp;&nbsp;<input name="reset" type="reset" id="reset" value="重 &nbsp; 置">
			<%}%></CENTER></FORM>
