 ...
 //  String user="dbuser"; 
  ...
  String user="sa"; 
   String password="1";
	  ...
	  String sql="select distinct fc.FileID,pf.FilesName,s.PaperName,s.PaperID,'PaperType'=case when s.Type='1' then '毕业论文' else '学年论文' end,'NowStatus'=s.Status from paperfile pf,filecontent fc,selectpaper s where s.StudentID='"+id+"' and s.StudentID=fc.StudentID and s.PaperID=fc.PaperID and fc.FileID=pf.FileID";
          ...
          <TD bgColor=#75c6c6>
            <TABLE cellSpacing=1 cellPadding=4 width="100%" border=0>
              <TBODY>
                <TR class=header>
                  <TD colSpan=4>
				    <div align="left">提示：点击编辑修改提交的文章,点击详情查看提交的文章</div></TD>
                </TR>
                ...
                <TR align=middle>
                  ...
                  <TD bgColor=#ffffff><%String nowstatus=rs.getString("NowStatus");//String status=rs.getString("Status");
				  if(!nowstatus.equals("5")){%><a href="editorces.jsp?action=editor&fid=<%=rs.getString("FileID")%>&sid=<%=id%>&pid=<%=rs.getString("PaperID")%>">[编辑]</A>
				  <%} else {%><a href="editorces.jsp?action=view&fid=<%=rs.getString("FileID")%>&sid=<%=id%>&pid=<%=rs.getString("PaperID")%>" target="_blank">[详情]</a><%}%></TD>
                </TR>
				<%}%>
              </TBODY>
          </TABLE></TD>
