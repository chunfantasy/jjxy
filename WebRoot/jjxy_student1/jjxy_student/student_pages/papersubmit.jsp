		  ...
		  for(int i=1;i<=count;i++)
		   { String a=String.valueOf(i);
			 ...
			 content=content.replaceAll("<","&lt;");
			  ...
			  {
		     values="'"+studentID+"','"+paperid+"','"+fileid;//+"',";
		     values=values+"','"+i+"','"+content+"'";
			 sql="insert into filecontent(StudentID,PaperID,FileID,ParaID,Content) values("+values+")";
			 values="'"+studentID+"','"+paperid+"','"+fileid;
			 }
			 else
			    if(action.equals("editor"))
				{
				values="' where StudentID='"+studentID+"' and PaperID='"+paperid+"' and FileID='"+fileid+"' and ParaID='"+i+"'";//+"',";
		     	sql="update filecontent set Content='"+content+values;
			  ...
			  try{
                stmt.executeUpdate(sql);
	             ...
	             conn.commit();
	                            ...
	                            conn.rollback();
	if(action.equals("submit")) {out.println("<br>"+filename+"提交失败。<br><br><a href='paperfile.jsp'>请点击这里返回提交页面</a><br><br>"); break;}
	else if(action.equals("editor")) {out.println("<br>"+filename+"修改失败。<br><br><a href='editorces.jsp?action=edtor&fid="+fileid+"&sid="+studentID+"&pid="+paperid+"'>请点击这里返回提交页面</a><br><br>"); break;}
	}
}
if(j==count)
  out.println("<br>"+filename+"提交成功，请给老师留言并等待老师确认。<br><br><a href='studentinfo.jsp'>[ 需要转入主页面请点击这里 ]</a><br><br><a href='../message.jsp?sid="+studentID+"'>需要给老师留言，请点击这里</a> <br><br>");
   stmt.close();
	conn.close();
