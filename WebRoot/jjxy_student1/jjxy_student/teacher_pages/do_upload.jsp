<%@ page contentType="text/html; charset=gb2312" language="java" import="java.sql.*,java.lang.*,java.util.*,com.jspsmart.upload.*,java.text.*" errorPage="" %>
...
String studentid=su.getRequest().getParameter("studentid");
String papertype=su.getRequest().getParameter("type");
//out.print(studentid);
//out.print("<br>");
//out.print(PaperID);
//out.print("<br>");
//out.print(papertype);
String sql1="select d.DepartmentName,sp.PaperName,s.StudentName from department d,selectpaper sp,student s where sp.StudentID='"+studentid+"' and sp.PaperID='"+PaperID+"' and sp.Type='"+papertype+"' and s.DepartmentID=d.DepartmentID and sp.StudentID=s.StudentID";
...
String sql="update studentpaper set PaperAddress='"+address+"' where StudentID='"+studentid+"' and PaperID='"+PaperID+"'";
//out.println(sql);
SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss"); 
java.util.Date currentTime_1 = new java.util.Date(); 
String sb = sdf.format(currentTime_1);
 CallableStatement stmt1= conn.prepareCall("{call EW_InsertMessage(?,?,?,?,?,?)}");
     	  stmt1.setString(1,studentid);
	      stmt1.setString(2,checkID);
		  stmt1.setInt(3,0);
		  stmt1.setString(4,"insert");
		  stmt1.setString(5,"<font color=red>系统提示：</font><br>&nbsp;&nbsp;&nbsp;论文已经被 <i>"+teacherName+"</i> 老师 于 "+sb+" 修改");
		  //stmt.setString(4,keyWord);
	     stmt1.registerOutParameter(6,java.sql.Types.VARCHAR);
...
try{
    ...
    conn.commit();
	//out.println(address);
	out.println("<br>论文上传成功。<br><br><a href='onlineguide.jsp?type="+papertype+"'>请点击这里返回主页面</a><br><br>");
  }catch(SQLException ex){
	conn.rollback();
	out.println("<br>论文上传成功，但更新数据库失败，请重新上传。<br><br><a href='uploda.jsp?sid="+studentid+"&pid="+PaperID+"&type="+papertype+"'>请点击这里重新上传</a><br><br>");
  }finally{
    rs.close();
	stmt.close();
	stmt1.close();
	conn.close();
     ...
     out.println("<br>论文上传失败，请重新上传。<br><br><a href='onlineguide.jsp?type=1'>请点击这里返回</a><br><br>");
 }
